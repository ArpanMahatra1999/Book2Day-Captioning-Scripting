...
# create document with title, weekday, authors, script, caption
def create_document(title, weekday, authors, script, caption):
    doc = Document()
...
def send_email_with_attachments(files):
    ...
    for file in files:
        filename = create_document(file['title'], file['weekday'], file['authors'], file['script'], file['caption'])
